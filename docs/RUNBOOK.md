...
Last updated: 2026-08-15 (multi-node infra bootstrap — both clusters live)
...
- `PVE_HOST` / `PVE_TOKEN` — legacy single-node runtime credential (DL360p,
  token `vmhub@pve!automation`). Still live; the multi-node path does not use it.
- **`VOSTRO_AUTOMATION_TOKEN` / `VOSTRO_OBSERVE_TOKEN`** — Vostro node tokens
  (2026-08-15)
- **`DL360P_AUTOMATION_TOKEN` / `DL360P_OBSERVE_TOKEN`** — DL360p node tokens
  (2026-08-15)
...
## 5b. Multi-node fleet — nodes, tokens, networking (T3 kill-gate PASSED 2026-08-15)

Two Proxmox nodes are wired into the vmhub fleet. Both run PVE 9.2.2. The
control plane reads the node registry from env (NodeConfig convention), one
client per node:

| Node id | Host | API base URL | Proxmox node | Storages |
|---|---|---|---|---|
| `vostro` | Dell Vostro 3681 (192.168.1.153) | `https://192.168.1.153:8006` | `pve` | local, local-lvm (thinpool) |
| `dl360p` | HP DL360p Gen8 (192.168.1.220) | `https://192.168.1.220:8006` | `vmhub` | vmhub (zfspool), local, local-lvm, iso-store |

Registry env convention (what lite + reaper read):
`VMHUB_NODES=<comma-separated node ids>` and per node
`VMHUB_NODE_<ID>_BASE_URL` (host:port) + `VMHUB_NODE_<ID>_TOKEN` (secret value).
Token values come from Doppler only — never from git.

API tokens (created via `pveum`, privsep=1, no expiry):

| Token ID | Scope |
|---|---|
| `vmhub@pve!vmhub-automation` (both nodes) | PVEVMAdmin + PVEAuditor on `/` — VM ops incl. destroy, clone, start/stop, config; read-only on top (node/storage/VM audit). |
| `vmhub@pve!vmhub-observe` (both nodes) | PVEAuditor on `/` — strictly read-only: node status + storage free (the RAM/disk headroom availability predicate) + VM audit. |
| `vmhub@pve!automation` (DL360p only) | Legacy single-node token (`/vms` PVEVMAdmin + `/storage` PVEAuditor + SDN.Use on vmbr1 zone). Kept for the PVE_HOST/PVE_TOKEN path; the new tokens are the fleet boundary. |

ACL model (learned the hard way on 2026-08-15): PVE evaluates a
priv-separated token's permissions as the **intersection** of the token's own
ACL roles and the owning user's ACL roles (`RPCEnvironment::compile_acl_path`).
So the user needs the union of both tokens' roles as a backing grant, AND each
token needs its own scoped role. Applied on both nodes:
`pveum aclmod / -user vmhub@pve -role PVEVMAdmin`, `-role PVEAuditor`;
automation token gets PVEVMAdmin + PVEAuditor on `/`; observe token gets
PVEAuditor on `/`. (A token-only ACL at `/` yields nothing — the user must
hold the same privileges at that path.)

Networking — isolated guest bridge on BOTH nodes (guests never touch the LAN):
`vmbr1` 10.10.10.1/24, `bridge-ports none`, ip_forward=1, iptables
`MASQUERADE` out vmbr0 for 10.10.10.0/24 + FORWARD accept rules. Vostro was
provisioned by `scripts/provision-vostro.sh` (idempotent, re-runnable — also
creates user/tokens/ACLs and checks NTP). DL360p's vmbr1 predates this task.

NTP: both nodes synced. Vostro `timedatectl` NTPSynchronized=yes; DL360p
`chronyc tracking` stratum 3 (offset <1ms). Lease expiry is wall-clock math —
keep NTP on.
